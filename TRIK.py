...
class script:
    ...
    def run(self) -> None:
        ...
        пока какой-либо из обработчиков не вызовет метод «quit».
        :return: None
        """
        return
    ...
    def timer(self, n: int):
        # TODO (непонятно, что за сигналы и как с ними работать)
        pass
...
class mailbox:
    ...
    def connect(self, ipAddress: str, port=8888) -> None:
        """
        ...
        :param ipAddress: IP-адрес робота, к которому происходит подключение
        :param port: Порт, по которому происходит подключение
        :return: None
        """
        return
    ...
    def newMessage(self):
        # TODO: очень нечеткая документация
        pass
...
class gamepad:
    ...
    def buttonWasPressed(self, buttonNumber: int) -> bool:
        ...
        :param buttonNumber: В качестве параметра необходимо указать номер кнопки — от 1 до 5.
        :return: bool
        """
        return False
    ...
    def wheel(self) -> int:
        ...
        Наклон кодируется числом от -100 до 100,
        -100 соответствует крайнему левому положению «руля», 100 — крайнему правому.
        :return: int
        """
        return 0
...
def getPhoto() -> list:
    """
    Функция возвращает одномерный массив байт, в который записаны пиксели
    изображения в формате rgb32, снятого с камеры (размер изображения — 160x120).
